...
async function dailyTask(){
  ...
  const bodyEn = 'We continue to the next phase today. Evening drops start at 18:00 (EET) 🔥'
  const titleFi = 'AI Spore Heräämispäivitys'
  const bodyFi = 'Jatkamme seuraavaan vaiheeseen tänään. Iltajulkaisut alkavat klo 18:00 (EET) 🔥'
  const text = `🌱 <b>${titleEn}</b>\n${bodyEn}\n—\n<b>${titleFi}</b>\n${bodyFi}`
  await send(text)
